...
class Part_Att(nn.Module):
    def __init__(self, channel, parts, **kwargs):
        ...
        self.parts = parts
        self.joint = get_corr_joint(parts)

        inter_channel = channel // 4

        self.fcn = nn.Sequential(
            nn.AdaptiveAvgPool12d(1),
    ...
    def forward(self, x):
        ...
        x_att = torh.split(x_att, 1, dim=-1)
...
class Part_Share_Att(nn.Module):
    def __init__(self, channel, parts, **kwargs):
        ...
        self.parts = parts
        self.joints = get_corr_joint(parts)
    ...
    def forward(self, x):
        ...
        x_att = [x_att[self.joints[i]].expan_as(x[:,:,:,i]) for i in range(v)]
        x_att = torch.stack(x_att, dim=-1)
        return self.relu(self.bn(x * x_att) + res)
...
class Channel_Att(nn.Module):
    def __init__(self, channel, **kwargs):
        ...
        self.fcn = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(channel//4, channel, kernel_sizw=1),
            nn.Softmax(dim=1)
        )
...
class Frame_Att(nn.Module):
    def __init__(self, channel, **kwargs):
        ...
        self.max_pool = nn.AdaptiveMaxPool12d|(1)
